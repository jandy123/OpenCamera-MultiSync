...
import net.sourceforge.opencamera.multisync.CameraController.CameraControllerManager2;
...
import net.sourceforge.opencamera.multisync.remotecontrol.BluetoothLeService;
import net.sourceforge.opencamera.multisync.remotecontrol.DeviceScanner;
import net.sourceforge.opencamera.multisync.remotecontrol.PeripheralRoleService;
import net.sourceforge.opencamera.multisync.UI.FolderChooserDialog;
import net.sourceforge.opencamera.multisync.UI.MainUI;
import net.sourceforge.opencamera.multisync.UI.ManualSeekbars;
...
import com.googleresearch.capturesync.CameraControllerSync;
...
import  com.googleresearch.capturesync.ImageMetadataSynchronizer;
...
public class MainActivity extends Activity {
	...
	public CameraControllerSync cameraControllerSync;
	...
	//private final int phaseConfigFile = R.raw.s20ultra_phaseconfig; //LUK:
	//private final int phaseConfigFile = R.raw.p20pro_master_phaseconfig; //LUK: phase sync 80ms period
	private final int phaseConfigFile = R.raw.p20pro_slave_phaseconfig; //LUK: phase sync 40ms period

	public boolean phase_sync = false;
	public boolean estimate_period = false;
	 ...
	 */
	public SoftwareSyncController softwareSyncController;
	...
	private PeripheralRoleService mBleServerService;
	private BluetoothLeService mBluetoothLeService;
	private volatile int bleCmd;
	private String mRemoteDeviceAddress;
	private String mRemoteDeviceType;
	private volatile boolean mRemoteConnected;
	...
	static private final String ACTION_SHORTCUT_CAMERA = "net.sourceforge.opencamera.multisync.SHORTCUT_CAMERA";
	static private final String ACTION_SHORTCUT_SELFIE = "net.sourceforge.opencamera.multisync.SHORTCUT_SELFIE";
	static private final String ACTION_SHORTCUT_VIDEO = "net.sourceforge.opencamera.multisync.SHORTCUT_VIDEO";
	static private final String ACTION_SHORTCUT_GALLERY = "net.sourceforge.opencamera.multisync.SHORTCUT_GALLERY";
	static private final String ACTION_SHORTCUT_SETTINGS = "net.sourceforge.opencamera.multisync.SHORTCUT_SETTINGS";
	...
	//LUK: ugly force left or right (different per phone!!!)
	//public static Stereo suffixSelection = Stereo.MONO; // no suffix, left suffix, right suffix
	public static Stereo suffixSelection = Stereo.LEFT;
	...
	private volatile boolean is_focused = false;
	private volatile float focus_dst = -1.0f;
	//private int server_delay = 100;
	//private int ble_latency = 2;
	private volatile boolean send_exp = true;

	private volatile long serverTime = Long.MAX_VALUE;

	private DeviceScanner ble_scanner;

	private long ntp_offset, ntp_sys_time, ntp_monotonic_time, ntp_round_trip;
	private boolean have_ntp_time = false;
	...
	private final ServiceConnection mServerServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName componentName, IBinder service) {
			...
			mBleServerService = ((PeripheralRoleService.LocalBinder) service).getService();
			if (!mBleServerService.initialize()) {
				Log.e(TAG, "Unable to initialize Bluetooth");
				finish();
			}
			// Advertizes service
			mBleServerService.startAdvertising();
		}
		...
		public void onServiceDisconnected(ComponentName componentName) {
			...
			handler.postDelayed(new Runnable() {
				public void run() {
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
						// BluetoothLeService requires Android 4.3+
						mBleServerService.startAdvertising(); //LUK:????
					}
				}
			}, 5000);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		takePhotoButton.setOnTouchListener(new OnTouchListener() {
		...
		//LUK: may be useful to do something on client & server when ble client connected & touching remote icon
		View remoteConnectedIcon = findViewById(R.id.kraken_icon);
		remoteConnectedIcon.setOnTouchListener(new OnTouchListener() {
			@Override
			public boolean onTouch(View view, MotionEvent motionEvent) {
				...
				//LUK: phase sync
				if(phase_sync) {
					//estimate_period = true;
					softwareSyncController.estimatePeriod();
					//estimate_period = false;
					return true;
				}
				*/

				if (!isBleServerConnected()) return true;
		...
		if (!has_done_first_time) {
			if (!is_test) {
				...
				alertDialog.setTitle("Open Camera MultiSync");
		...
		//LUK: phase sync
		if(sharedPreferences.getString("preference_sync_type", "preference_sync_simple").equals("preference_sync_phase"))
			phase_sync = true;
		...
		//LUK: copy executables so that they can be executed; search for run_telnet.sh
		copyAssets();
	...
	public void playShutterSound() {
		preview.getCameraController().playShutterSound();
	}
	...
	public void savePicture(Image image) {
		...
		getApplicationInterface().onPictureTaken(bytes, new Date());
	}

	public void broadcastFile(File file, Bitmap thumbnail) {
		...
		getStorageUtils().broadcastFile(file, true, false, true);
	}
...
*/
 ...
 /** Create {@link #cameraController}, and subscribe to status change events. */
  private void initCameraController() {
    ...
    cameraControllerSync =
        new CameraControllerSync(
            null,
            null,
				//new Size(4000, 3000), //LUK: fix this; samsung
 				new Size(3648, 2736), //LUK: fix this; huawei
...
private void startSoftwareSync() {
	...
	periodCalculator = new PeriodCalculator(this, this.getPreview());

    try {
      softwareSyncController =
          new SoftwareSyncController(this, phaseAlignController, periodCalculator);
...
private PhaseConfig loadPhaseConfigFile() throws JSONException {
	...
	void checkServerName() {
		//LUK:!!! check if server address has changed; this assumes client was previously connected with the server
		//so that actual name matches
		final SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		final String preference_remote_device_name = sharedPreferences.getString(PreferenceKeys.RemoteName, "undefined");
		final String preference_remote_server_name = sharedPreferences.getString(PreferenceKeys.server_name, "undefined");

		if(isBleClientConnected()) return;

		if(sharedPreferences.getBoolean(PreferenceKeys.EnableRemote, true) /*&& !preference_remote_device_name.equals("undefined") && !preference_remote_server_name.equals("undefined")*/) {
			final BluetoothManager bluetoothManager = (BluetoothManager) getSystemService(Context.BLUETOOTH_SERVICE);
			final BluetoothAdapter mBluetoothAdapter = bluetoothManager.getAdapter();

			//LUK: try connect without scan; needs  devices to be bound apparently...
			if(preference_remote_server_name != null && preference_remote_device_name != null) {
				startRemoteControl();
				return;
			}
			
			BluetoothAdapter.LeScanCallback  mLECallback = new BluetoothAdapter.LeScanCallback() {

						@Override
						public void onLeScan(final BluetoothDevice device, int rssi, byte[] scanRecord) {
							String name = device.getName();
							if (name != null && name.equals(preference_remote_server_name)) {
								if (!preference_remote_device_name.equals(device.getAddress())) {
									//SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
									SharedPreferences.Editor editor = sharedPreferences.edit();
									editor.putString(preference_remote_device_name, device.getAddress());
									editor.commit(); //LUK: not working???
	...
	void checkServerNameThread() {
		//LUK:!!! check if server address has changed; this assumes client was previously connected with the server
		//so that actual name matches

		ThreadedScan scanner = new ThreadedScan();
		scanner.start();
	}
...
//		}
	...
	public void zoomIn() {
		if(mBleServerService != null)
			mBleServerService.sendValue(BluetoothLeService.COMMAND_UP);
		mainUI.changeSeekbar(R.id.zoom_seekbar, -1);
	}
	
	public void zoomOut() {
		if(mBleServerService != null)
			mBleServerService.sendValue(BluetoothLeService.COMMAND_DOWN);
		mainUI.changeSeekbar(R.id.zoom_seekbar, 1);
	}
            ...
            if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(action)) {
                ...
                */
				...
				if(isBleClientConnected()) {
					mBluetoothLeService.setActivity((MainActivity)context);
				}
                ...
                switch (command) {
					case BluetoothLeService.COMMAND_FOCUS_EXP:
						//LUK: phase sync
						if(!phase_sync) {
							setBleCmd(1);
							takingPicture = true;
						...
						break;
					case BluetoothLeService.COMMAND_SHUTTER: //LUK: never fired anymore, see BLEService
						setBleCmd(2);
						// Easiest - just take a picture (or start/stop camera)
						MainActivity.this.takePicture(false);
						break;
    ...
    public boolean remoteConnected() {
		/*if( true )
			return true; // test*/
        //LUK: replaced to show on both server & client
	...
	/**
	 * Starts or stops the BLE server
	 */
	private void startServer() {
		if( MyDebug.LOG )
			Log.d(TAG, "BLE server service start check...");
		if( Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2 ) {
			// BluetoothLeService requires Android 4.3+
			return;
		}
		Intent gattServiceIntent = new Intent(this, PeripheralRoleService.class);
		if ( serverEnabled() ) {
			if( MyDebug.LOG )
				Log.d(TAG, "BLE server enabled, starting service");
			bindService(gattServiceIntent, mServerServiceConnection, BIND_AUTO_CREATE);
		} else {
			if( MyDebug.LOG )
				Log.d(TAG, "BLE server disabled, stopping service");
			// Stop the service if necessary
			try{
				...
				if( MyDebug.LOG )
					Log.d(TAG, "BLE Server service was not running, that's fine");
			}
		}
	}
    ...
    protected void onResume() {
		...
		// if BLE server is enabled, then start the background BLE service
		startServer();
    ...
    public void clickedTakePhoto(View view) {
		...
		//LUK:take photo
		if(isBleServerConnected()) {//server side
			mBleServerService.sendValue(BluetoothLeService.COMMAND_FOCUS_EXP);

			takingPicture = true;
			SystemClock.sleep(getServerDelay()); //LUK: required!!!
		}
	...
	public int getServerDelay() {
		SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		return Integer.parseInt(sharedPreferences.getString(PreferenceKeys.serverDelay, "200"));
	}

	public int getBleLatency() {
		SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		return Integer.parseInt(sharedPreferences.getString(PreferenceKeys.bleLatency, "2"));
	}

	public void showToast(String msg) {
		preview.showToast(msg, true);
	}
	...
	public boolean sendExpToClient() {
	...
	public void sendBleValue(int val) {
		if(isBleServerConnected()) {
			mBleServerService.sendValue(val);
		}
	}

	public void sendBleShutdown() {
		if(isBleServerConnected()) {
			mBleServerService.sendShutdown();
		}
	}

	public void sendBleServerTime(long val) {
		if(isBleServerConnected()) {
			mBleServerService.sendServerTime(val);
		}
	}

	public void sendBleFloatValue(float val) {
		if(isBleServerConnected()) {
			mBleServerService.sendFloat(val);
			//SystemClock.sleep(50); //LUK: wait to not overwrite
		}
	}

	public void sendBleFocusRegion(float x, float y) {
		if(isBleServerConnected()) {
			mBleServerService.sendFocusRegion(x, y);
		}
	}

	public void sendBleISOExpTime(int iso, long exptime) {
		if(!takingPicture && isBleServerConnected()) {
			mBleServerService.sendIsoTime(iso, exptime);
			//SystemClock.sleep(50); //LUK: wait to not overwrite
		}
	}

	public void sendBleCmdNtpSync() {
		if(isBleServerConnected()) {
			mBleServerService.sendValue(12);
			//SystemClock.sleep(50); //LUK: wait to not overwrite
		}
	}

	public synchronized int getBleCmd() {
			return bleCmd;
	}

	public synchronized void setBleCmd(int v) {
			bleCmd = v;
	}

	public synchronized long getServerTime() { return serverTime; }

	public synchronized void setServerTime(long v) {
		serverTime = v;
	}

	public void setZoomBle(int val) {
		SeekBar zoomSeekBar = findViewById(R.id.zoom_seekbar);
		zoomSeekBar.setProgress(val);
	}

	public void setShutdown() {
			Context ctx = this;
			Intent i = ctx.getPackageManager().getLaunchIntentForPackage("nl.matthijsvh.screenoff");
			if(i==null) return;
			try{
				ctx.startActivity(i);
			}
	...
	public void setFocusBle(float val) {
		if(preview.getCameraController().getFocusValue().equals("focus_mode_manual2")) {

			preview.setFocusDistance(val, true);

/*
//LUK: taken out for now!
			boolean is_target_distance = false; //LUK:????
			final SeekBar focusSeekBar = findViewById(is_target_distance ? R.id.focus_bracketing_target_seekbar : R.id.focus_seekbar);

			float scaling = val / preview.getMinimumFocusDistance();
			double frac = ManualSeekbars.seekbarScalingInverse(scaling);
			int progress = (int)(frac * focusSeekBar.getMax());

			focusSeekBar.setProgress(progress);
*/

		}
		else if(preview.getCameraController().getFocusValue().equals("focus_mode_auto")) {
			preview.requestAutoFocus();
		}
	}

	public void setFocusRegion(float x, float y) {
/*
		//LUK: estimate disparity; taken out since focus distance is not available and is unreliable
		see https://dibyendu-biswas.medium.com/stereo-camera-calibration-and-depth-estimation-from-stereo-images-29d87bc702f3
		https://stackoverflow.com/questions/36349870/find-the-dimensions-height-width-of-an-object-using-camera
		float disparity = preview.getCameraController().getDisparity(getFocusDst());
		preview.showToast("Disparity: " + disparity, false);
*/
		//simulate touch
		MotionEvent event = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_DOWN, x, y, 0);
		preview.touchEvent( event);
		SystemClock.sleep(10);
		event = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_UP, x, y, 0);
		preview.touchEvent( event);
	}

	public void setIsoBle(int iso) {
		//if(preview.getCameraController().getFocusValue().equals("focus_mode_manual2")) { //LUK: allow also when auto focus?
			preview.setISO(iso, false);
		//}
	}

	public void setExpTimeBle(long time) {
		//if(preview.getCameraController().getFocusValue().equals("focus_mode_manual2")) { //LUK: allow also when auto focus?
			preview.setExposureTime(time, false);
		//}
	}

	public void setFocusDst(float v) { focus_dst = v; }

	public float getFocusDst() {
		return focus_dst;
	}

	public boolean isFocused() { return is_focused; }

	//LUK: make sure that serves focused, see CameraController2, sync procedure
	public void setFocused(boolean f) { is_focused = f; }

	public boolean isBleServer() {
		return mBleServerService != null;
	}

	public boolean isBleServerConnected() {
		if(mBleServerService != null) {
			return mBleServerService.isBleServerConnected();
		}
		return false;
	}

	public boolean isBleClient() {
		return mBluetoothLeService != null;
	}

	public boolean isBleClientConnected() {
		if(mBluetoothLeService != null)
		...
		public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
			...
			switch( key ) {
                ...
                case PreferenceKeys.EnableRemote:
                    ...
                    startRemoteControl();
                    break;
				case PreferenceKeys.EnableServer:
					startServer();
					break;
				case PreferenceKeys.serverDelay:
					//server_delay = Integer.parseInt(sharedPreferences.getString(PreferenceKeys.serverDelay, "100"));
					break;
				case PreferenceKeys.bleLatency:
					//ble_latency = Integer.parseInt(sharedPreferences.getString(PreferenceKeys.bleLatency, "2"));
					break;
				case PreferenceKeys.send_focus_exposure:
					send_exp = sharedPreferences.getBoolean(PreferenceKeys.send_focus_exposure, true);
					break;
               case PreferenceKeys.RemoteName:
				...
				case PreferenceKeys.WaterType:
					...
					break;
					//LUK: phase sync
				case PreferenceKeys.syncType:
					boolean phase_sync_enabled = false;
					if(sharedPreferences.getString("preference_sync_type", "preference_sync_simple").equals("preference_sync_phase"))
						phase_sync_enabled = true;

					if(phase_sync ^ phase_sync_enabled) {
						SharedPreferences.Editor editor = sharedPreferences.edit();
						editor.commit();
						editor.apply();
						preview.showToast(null, "Restarting...");
     ...
     */
    void lockScreen() {
		findViewById(R.id.locker).setOnTouchListener(new OnTouchListener() {
			...
			if( preview.supportsZoom() ) {
				...
				zoomSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
					...
					public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
						...
						if(mBleServerService != null)
							mBleServerService.sendZoom(progress);
			...
			if( preview.supportsISORange()) {
				...
				iso_seek_bar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
					...
					public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
						...
						preview.setISO( manualSeekbars.getISO(progress), true );
				...
				if( preview.supportsExposureTime() ) {
					...
					exposure_time_seek_bar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
						...
						public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
							...
							preview.setExposureTime( manualSeekbars.getExposureTime(progress), true );
						}
	...
	public boolean remoteEnabled() {
		...
		//LUK: do not overwrite mRemoteDevice
	...
	public boolean serverEnabled() {
		...
		SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		boolean server_enabled = sharedPreferences.getBoolean(PreferenceKeys.EnableServer, false);
	...
	public String getSyncType() {
		final SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		return sharedPreferences.getString("preference_sync_type", "preference_sync_simple");
	}

	public String getNtpServer() {
		final SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		return sharedPreferences.getString("preference_ntp_server", "pool.ntp.org");
	}
	...
	private void copyAssets() {
		...
		String[] files = null;
		try {
			files = assetManager.list("telnet");
		} catch (IOException e) {
		...
		for(String filename : files) {
			...
			try {
				in = assetManager.open("telnet/" + filename);
	...
	public void syncNTP() {
		Thread thread = new Thread(new Runnable() {
			@Override
			public void run() {
				SntpDsense client = new SntpDsense();

				boolean SntpSuceeded = client.requestTime(getNtpServer(), 1000);

				if (SntpSuceeded) {
					ntp_offset = client.getNtp_clockoffset();
					ntp_sys_time = client.get_ntp_update_sys_time();
					ntp_monotonic_time = client.get_ntp_update_monotonic_time();
					ntp_round_trip = client.get_round_trip_time();
					have_ntp_time = true;
				}
			}
		});
		...
		if (have_ntp_time)
			showToast("NTP offset: " + ntp_offset + "  round trip: " + ntp_round_trip);
		else
			showToast("NTP request error");
	}

	public boolean haveNtpTime() { return have_ntp_time; }

	public long ntpOffset() { return ntp_offset; }

	//LUK: phase sync
	  public void notifyCapturing(String name) {
		runOnUiThread(
			() -> {
				preview.showToast(null,"Capturing " + name + "..." );
			});
	  }
	  ...
	  public void notifyCaptured(String name) {
		...
		runOnUiThread(
			() -> {
				preview.showToast(null, "Captured " + name);
			});
	  }
		/**
		...
		public String getSyncStatusText() {
			return softwareSyncController.getSyncStatus();
		}
	  ...
	  public Pair<String, Boolean> getPhaseError() {
		  return phaseAlignController.getPhaseError();
	  }
		  ...
		  CaptureRequest.Builder builder =
			  cameraControllerSync
				  ...
				  .makeFrameInjectionRequest(
					  desiredExposureTimeNs, cameraControllerSync.getOutputSurfaces());
		  preview.getCameraController().capture(builder.build());
		  /*
		  captureSession.capture(
			  builder.build(), cameraControllerSync.getSynchronizerCaptureCallback(), preview.getCameraController().getHandler());
	  ...
	  public void setUpcomingCaptureStill(long upcomingTriggerTimeNs) {
		cameraControllerSync.setUpcomingCaptureStill(upcomingTriggerTimeNs);
		double timeTillSec =
		...
		/*
			runOnUiThread(
			() -> {
				preview.showToast(null, String.format("Capturing in %.2f seconds", timeTillSec));
			});
		 */
	  }

	  public void set2aAndUpdatePreview(long sensorExposureTimeNs, int sensorSensitivity) {
		  /*
