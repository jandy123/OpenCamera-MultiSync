package net.sourceforge.opencamera.multisync.CameraController;
...
import net.sourceforge.opencamera.multisync.SntpDsense;
...
public class CameraController2 extends CameraController {
	...
	private volatile long cap_time;
	private volatile long cap_avg = 0L;
	private int tp = 0;
	...
	public CameraController2(Context context, int cameraId, final ErrorCallback preview_error_cb, final ErrorCallback camera_error_cb) throws CameraControllerException {
		...
		//thread.setPriority(Thread.MAX_PRIORITY); //LUK: replaced with urgent audio below; improvement???
		thread.start();
		handler = new Handler(thread.getLooper());
		android.os.Process.setThreadPriority(thread.getThreadId(), android.os.Process.THREAD_PRIORITY_URGENT_AUDIO);
	...
	public float getDisparity(float dst_reciprocal) {
		final float base = 10.0f;
		float focalLength = characteristics.get(CameraCharacteristics.LENS_INFO_AVAILABLE_FOCAL_LENGTHS)[0];

		return focalLength * base * dst_reciprocal;
	}
...
//		Frame duration is in nanoseconds.  Using min to be safe.
	...
	public CameraDevice getCameraDevice() { return camera; }
	...
	private void createCaptureSession(final MediaRecorder video_recorder, boolean want_photo_video_recording) throws CameraControllerException {
		...
		try {
			...
			class MyStateCallback extends CameraCaptureSession.StateCallback {
				...
				public void onConfigured(@NonNull CameraCaptureSession session) {
					...
					synchronized( background_camera_lock ) {
						...
						if(mainActivity.phase_sync) {
							List<Integer> targetIndices = new ArrayList<>();
							List<Surface> imageSurfaces = mainActivity.cameraControllerSync.getOutputSurfaces();
							for (int i = 0; i < imageSurfaces.size(); i++) {
								previewBuilder.addTarget(imageSurfaces.get(i));
								targetIndices.add(i);
							}
							previewBuilder.setTag(new ImageMetadataSynchronizer.CaptureRequestTag(targetIndices, null));

							mainActivity.cameraControllerSync.configure(camera);
						}
						//end LUK
        	...
        	if( video_recorder != null ) {
				if( supports_photo_video_recording && !want_video_high_speed && want_photo_video_recording ) {
					surfaces = new ArrayList<>(Arrays.asList(preview_surface, video_recorder_surface, imageReader.getSurface()));
				}
				else {
					surfaces = new ArrayList<>(Arrays.asList(preview_surface, video_recorder_surface));
				}
				// n.b., raw not supported for photo snapshots while video recording
        	}
    		else if( imageReaderRaw != null ) {
        		surfaces = new ArrayList<>(Arrays.asList(preview_surface, imageReader.getSurface(), imageReaderRaw.getSurface()));
    		}
    		else {
				//LUK: phase sync
				MainActivity mainActivity = ((MainActivity)context);
				if(mainActivity.phase_sync) {
					surfaces = new ArrayList<>(Arrays.asList(preview_surface /*, imageReader.getSurface()*/));
					surfaces.addAll(mainActivity.cameraControllerSync.getOutputSurfaces());
				}
				else
					surfaces = new ArrayList<>(Arrays.asList(preview_surface, imageReader.getSurface() ));
			}
		...
		synchronized( background_camera_lock ) {
			...
			try {
				...
				if( !previewIsVideoMode ) {
					...
					captureSession.stopRepeating();
					//LUK: added; can this improve sync ???
...
/*
					//LUK: wait focus: still creates problems!!!
					for(int i=0; i<200; i++) {
						if (mActivity.isFocused()) break;
						SystemClock.sleep(10);
					}
					//reset focus
					mActivity.setFocused(false);
*/
					//LUK:!!!  force sync
					CaptureRequest req = stillBuilder.build();
					//SystemClock.sleep(2000);

					if(mActivity.getSyncType().equals("preference_sync_simple")) {

						if (mActivity.isBleServerConnected()) {
							mActivity.sendBleValue(BluetoothLeService.COMMAND_SHUTTER);
							SystemClock.sleep(mActivity.getBleLatency()); //LUK:!!! small delay
							//mActivity.showToast("Time: " + System.currentTimeMillis());
						} else if (mActivity.isBleClientConnected()) {
							//while (mActivity.getBleCmd() < 2) ;
							for(int i = 0; i< 2000; i++)  { //LUK: bug? Why not infinite loop as above?
								if (mActivity.getBleCmd() < 2) {
									SystemClock.sleep(1); //LUK:!!! small delay
								}
								else break;
							}
							//mActivity.showToast("Time: " + System.currentTimeMillis());
					...
					else if(mActivity.getSyncType().equals("preference_sync_ble")) {

						if (mActivity.isBleServerConnected()) {

							//LUK: prev. values tried: 60, 55, 50; best seems to be 55 or 50; has to do with connection interval, see below comment
							final long delay = 55; //ms; android BLE connection interval which is >15ms, see requestConnectionPriority in BluetoothLEService
							for (int i = 0; i < 10; i++) {
								mActivity.sendBleServerTime(SystemClock.elapsedRealtime());
								SystemClock.sleep(delay);
							}

							long theActualTime = SystemClock.elapsedRealtime();
							theActualTime += delay;
							mActivity.sendBleServerTime(theActualTime /*+ cap_avg*/);
							while (theActualTime > SystemClock.elapsedRealtime()) {
								//SystemClock.sleep(1);
								final long end = System.nanoTime() + 1000000L;
								while(System.nanoTime() < end) ;
							}
							SystemClock.sleep(mActivity.getBleLatency()); //LUK: additional sleep on server side
						} else if (mActivity.isBleClientConnected()) {

							long diff = Long.MAX_VALUE;
							for (int i = 0; i < 10; i++) {
								long stime = SystemClock.elapsedRealtime();
								for (int j = 0; j < 2000; j++) { //LUK: bug? Why not infinite loop as above?
									long st = mActivity.getServerTime();
									if (st == Long.MAX_VALUE) {
										SystemClock.sleep(1);
										continue;
									}
									long cdiff = SystemClock.elapsedRealtime() - st;
									if (diff > cdiff) diff = cdiff;
									mActivity.setServerTime(Long.MAX_VALUE);
									break;
								}
							}

							for (int i = 0; i < 2000; i++) { //LUK: bug? Why not infinite loop as above?
								if (mActivity.getServerTime() > SystemClock.elapsedRealtime() - diff /*+ cap_avg*/) {
									//SystemClock.sleep(1);
									final long end = System.nanoTime() + 1000000L;
									while(System.nanoTime() < end) ;
								}
							...
							mActivity.setServerTime(Long.MAX_VALUE);
							//mActivity.showToast("Server-client time offset: " + diff);
						}

					}
					else /*if(mActivity.getSyncType().equals("preference_sync_ntp"))*/ {

						if(!mActivity.haveNtpTime())
							mActivity.syncNTP();

						if(mActivity.haveNtpTime()) {
							if (mActivity.isBleServerConnected()) {

								//long theActualTime = ntp_offset + ntp_sys_time - ntp_monotonic_time;
								//while ( ((theActualTime + SystemClock.elapsedRealtime()) % 1000L) != 0L ) {
								//while( (ntp_offset + System.currentTimeMillis()) % 1000L !=0) {
									//final long end = System.nanoTime() + 1000000L;
									//while(System.nanoTime() < end) ;
								//}

								final long ntp_offset = mActivity.ntpOffset();

								//SystemClock.sleep(1000L - ((ntp_offset + System.currentTimeMillis()) % 1000L) );

								long theActualTime = ntp_offset + System.currentTimeMillis();
								theActualTime += 500L;
								mActivity.sendBleServerTime(theActualTime); //+ cap_avg

								while(theActualTime > ntp_offset + System.currentTimeMillis()) {
									//SystemClock.sleep(1);
									final long end = System.nanoTime() + 1000000L;
									while(System.nanoTime() < end) ;
								}
								...
								for (int i = 0; i < 2000; i++) { //LUK: bug? Why not infinite loop as above?
									if (mActivity.getServerTime() >  ntp_offset + System.currentTimeMillis()) { //+ cap_avg
										//SystemClock.sleep(1);
										final long end = System.nanoTime() + 1000000L;
										while(System.nanoTime() < end) ;
									}
					...
					if(mActivity.haveNtpTime() && (mActivity.isBleServerConnected() || mActivity.isBleClientConnected())) {
						sync_time = mActivity.ntpOffset() + System.currentTimeMillis();
					}
					else sync_time = 0L;

					//LUK:???
				//cap_time = SystemClock.uptimeMillis();
				//cap_time = SystemClock.elapsedRealtime();
				cap_time = SystemClock.elapsedRealtimeNanos();

					captureSession.capture(req, previewCaptureCallback, handler);
					//pending_request_when_ready = stillBuilder.build();
					//LUK: taken our, replace with above
	...
	private final CameraCaptureSession.CaptureCallback previewCaptureCallback = new CameraCaptureSession.CaptureCallback() {
		...
		public void onCaptureStarted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, long timestamp, long frameNumber) {
			...
			if( getRequestTagType(request) == RequestTagType.CAPTURE ) {
				//LUK: show time duration
				MainActivity mActivity = (MainActivity) context;
				//final long cap_delay = TimeUnit.NANOSECONDS.toMillis(timestamp) - cap_time;
				//final long cap_delay = SystemClock.elapsedRealtime() - cap_time;
				final long cap_delay = TimeUnit.NANOSECONDS.toMillis(timestamp - cap_time);
				cap_avg = (3L*cap_avg + 7L*cap_delay)/10L;

				if (mActivity.isBleServerConnected() || mActivity.isBleClientConnected()) {
					String msg = new String();
					if (sync_time != 0) {
						final int sec = (int) ((sync_time / 1000) % 60);
						msg = msg + "Sync time: " + sec + " : " + (sync_time % 1000);
					}
					msg = msg + " Capture delay: " + cap_delay;
		...
		public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, @NonNull TotalCaptureResult result) {
			...
			//LUK: phase sync
			MainActivity mainactivity = (MainActivity) context;
			if(mainactivity.phase_sync && getRequestTagType(request) == null) {
				mainactivity.cameraControllerSync.getSynchronizerCaptureCallback().onCaptureCompleted(session, request, result);
			}
			////
			...
			if (MyDebug.LOG) {
				if (getRequestTagType(request) == RequestTagType.CAPTURE) {
					...
					//((MainActivity)context).showToast("Focus: " + result.get(CaptureResult.LENS_FOCUS_DISTANCE));
				}
			}

			//LUK:
			//((MainActivity)context).setFocusDst(result.get(CaptureResult.LENS_FOCUS_DISTANCE));
...
//			}
			...
			//LUK: reset taking picture
			((MainActivity) context).setTakingPicture(false);

/*
			//LUK:
			if (tp == 1 && (((MainActivity) context).isTakingPicture()) && (((MainActivity) context).isBleClientConnected() || ((MainActivity) context).isBleServerConnected())) {
				((MainActivity) context).setTakingPicture(false);
				tp = 0;
				...
				MainActivity mActivity = (MainActivity) context;
				if (mActivity.isBleServerConnected()) {
					//server_millis = System.currentTimeMillis();
					//server_millis = System.nanoTime(); //LUK: use with timestamp in oncapturestarted
					mActivity.sendBleValue(BluetoothLeService.COMMAND_SHUTTER);
					SystemClock.sleep(mActivity.getBleLatency()); //LUK:!!! small delay
					//server_millis = System.currentTimeMillis();
				}
					...
					while (mActivity.getBleCmd() < 2) ;
					//client_millis = System.currentTimeMillis();
					//client_millis = System.nanoTime(); //LUK: use with timestamp in oncapturestarted
				}
			}
 */
...
/*
						//LUK:??? send focus; unreliable replaced with sendfocusregion
						if(af_state == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED)
							((MainActivity) context).setFocusDst(result.get(CaptureResult.LENS_FOCUS_DISTANCE));
*/
						/*
							//LUK:!!! focus mirror to client
							MainActivity mActivity = (MainActivity) context;

float dst = result.get(CaptureResult.LENS_FOCUS_DISTANCE);
mActivity.showToast("" + dst);
							mActivity.setFocused(true);
							if (mActivity.sendExpToClient()) {
								if (mActivity.isBleServerConnected()) {
									//float dst = camera_controller.getFocusDistance();
									//float dst = mActivity.getFocusDst();
									//float dst = result.get(CaptureResult.LENS_FOCUS_DISTANCE);
									//setFocusDistance(dst, true);
									if (MyDebug.LOG)
										Log.d(TAG, "Focus distance: " + dst);

									mActivity.sendBleFloatValue(dst);
									//mActivity.showToast(""+dst);
								}
							}
						}
