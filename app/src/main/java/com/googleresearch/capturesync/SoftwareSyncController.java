...
public class SoftwareSyncController implements Closeable {
  ...
  public SoftwareSyncBase softwareSync;
  ...
  public SoftwareSyncController(
      MainActivity context, PhaseAlignController phaseAlignController, PeriodCalculator periodCalculator) {
  ...
  private void setupSoftwareSync() {
      ...
      clientRpcs.put(
          ...
          payload ->
              context.runOnUiThread(
                  () -> mSyncStatus = softwareSync.getName() + ": Waiting for Leader"));
      clientRpcs.put(
          SyncConstants.METHOD_MSG_SYNCING,
          payload ->
              context.runOnUiThread(
                  () -> mSyncStatus = softwareSync.getName() + ": Waiting for Sync"));
      clientRpcs.put(
    ...
    if (isLeader) {
      context.runOnUiThread(
          () -> {
            mSyncStatus = "Leader : " + softwareSync.getName();
          });
    } else {
      context.runOnUiThread(
          () -> {
            mSyncStatus = "Client : " + softwareSync.getName();
          });
    }
  }
  ...
  public void estimatePeriod() {
    ...
    AsyncTask.execute(new Runnable() {
      ...
      public void run() {
        ...
        context.estimate_period = true;
        long periodNs = 0L;
        try {
          periodNs = mPeriodCalculator.getPeriodNs();
          Log.i(TAG, "Calculated frames period: " + periodNs);
        } catch (InterruptedException e) {
          Log.e(TAG, "Failed calculating frames period: ", e);
        }
        context.estimate_period = false;
        if(periodNs != 0L)
          context.getPreview().showToast(null, "Period: " + periodNs);
  ...
  public String getSyncStatus() { return mSyncStatus; }
